...
/*
 * Data Definitions
 * Most of it was taken from a perl mail parser from Benjamin Elijah Griffin
 */

static const struct {
	char const *type, *exts[7];
} type2exts[] = {
	// From /etc/mime.types
	...
	{ "text/iuls", { "uls", } },
	{ "text/mathml", { "mml", } },
	{ "text/plain", { "txt", "asc", "text", "pot", "brf", } },
	{ "text/richtext", { "rtx", } },
...
/*
 * Simple conversion from filename to mime_type
 */
...
{
	char const *c, *ext = NULL;
	for (c = fname; *c; c++) {
		if (*c == '.') ext = c+1;
	}
	if (ext && *ext) return ext2mime_type(ext);
	return NULL;
}
